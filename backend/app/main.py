...
from .models import (
    ...
    ProcessS3Request, ProcessFileRequest, ProcessTextRequest, ProcessResponse,
    QuestionCreate, QuestionResponse, BatchQuestionsRequest, BatchQuestionsResponse,
    HealthResponse, SubjectsResponse, StatsResponse, ErrorResponse,
    AdvancedQueryRequest, SuccessResponse
)
...
async def query_questions(request: QueryRequest):
    ...
    try:
        ...
        if request.subject:
            results = chroma_client.search_by_subject(
                request.question, 
                request.subject, 
                request.top_k
            )
...
async def process_file(file: UploadFile = File(...), namespace: str = "ssc-questions"):
    ...
    try:
        ...
        import asyncio
        asyncio.create_task(
            process_file_background(job_id, temp_path, namespace)
        )
...
async def process_text(request: ProcessTextRequest):
    ...
    try:
        ...
        import asyncio
        asyncio.create_task(
            process_text_background(job_id, request.text_content, request.namespace)
        )
...
async def health_check():
    ...
    try:
        ...
        count = chroma_client.get_collection_stats()
        db_status = "healthy"
    except Exception as e:
